...
Kafka 같은 분산 메시지 브로커는 이 프로젝트에 맞지 않는다. 방송 소스는 군포 한 곳, 서버는 한 대, 소비자는 소수 단말이며, 오디오는 늦으면 버리는 구조라 메시지를 보존했다 다시 읽을 이유가 없다. 1GB RAM에 무거운 JVM을 얹는 부담만 생긴다.

대신 서버 코드가 스스로 상태를 드러내는 가벼운 모니터링을 둔다. 감시 지점은 세 곳이다.
